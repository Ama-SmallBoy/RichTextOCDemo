...
//    RZRichTextConfigureManager.manager.overrideUserInterfaceStyle = RZUserInterfaceStyleDark;
    // 富文本输入框
    self.textView = [[RZRichTextView alloc] initWithFrame:CGRectMake(0, 0,CGRectGetWidth(self.view.frame), CGRectGetHeight(self.view.frame))];
    self.textView.font = [UIFont systemFontOfSize:17];
    self.textView.backgroundColor = UIColor.rz_colorCreaterStyle(0, [UIColor whiteColor], [UIColor whiteColor]);
    
//    NSString * filePath = [[NSBundle mainBundle] pathForResource:@"content" ofType:@"html"];
//    
//     NSString* htmlContent = [[NSString alloc]initWithContentsOfFile:filePath encoding:NSUTF8StringEncoding error: nil];
//
//    NSAttributedString * attributedString = [NSAttributedString htmlString:htmlContent];
//    
...
//    }
    ...
    [self.textView rz_colorfulConfer:^(RZColorfulConferrer * _Nonnull confer) {
        ...
        NSString *filePath =[resourcePath stringByAppendingPathComponent:@"content.html"];
...
- (IBAction)convertStringToHtmlAction:(UIButton *)sender {
//    NSLog(@"======%@",[NSAttributedString htmlString:<#(NSString *)#>])
